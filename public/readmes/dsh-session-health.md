...
> 已知限制：DSH 0.1.5-rc.1 下 deep 模式依赖的 `@deepseek-ai/dsh-session-persistence-jsonl` tarball 仍不含 `src/`，根入口仍不导出 zstd API，deep 降级 `decoder-unavailable`；frame-level 扫描不受影响（已报 dsh-external/issues，该组织为组织基础设施，保留）。


## 安装

DSH 0.1.5-rc.1（npm）下，插件通过 `dsh plugin --profile <profile> add <source>` 安装，source 支持 GitHub 仓库或 npm pack tarball。
