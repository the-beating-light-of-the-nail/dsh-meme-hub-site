...
> 已知限制：DSH 0.1.2-rc.1 下 deep 模式依赖的 `@deepseek-ai/dsh-session-persistence-jsonl` tarball 仍不含 `src/`，根入口仍不导出 zstd API，deep 降级 `decoder-unavailable`；frame-level 扫描不受影响（已报 dsh-external/issues，该组织为组织基础设施，保留）。
