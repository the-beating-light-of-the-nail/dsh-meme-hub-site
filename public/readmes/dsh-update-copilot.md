...
<p align="center">
  <img src="https://raw.githubusercontent.com/hezhongtang/dsh-update-copilot/1e0768ae750654724942a043734ee440a9961f07/assets/popup.png" width="480" alt="The Update Copilot popup: the DeepSeek Harness core card collapsed, plugins merged across profiles into an Updates-available section with one-click update buttons, and the Up-to-date section folded away." />
</p>
...
| 🤖 **Agent tools** | `update_copilot_scan` / `update_copilot_brief` / `update_copilot_preflight` / `update_copilot_update` — ask your agent *"any updates?"* and get an honest, data-backed answer. Scans are package-centric (one row per package, merged across profiles); inferred mount relationships are presentation-only, while official packages are report-only and each direct dependency keeps its own update policy. `profile` is optional on brief/preflight/update: without it, a brief covers every eligible profile that has the package, preflight evaluates every profile that has it, and an update runs only across those eligible profiles |
