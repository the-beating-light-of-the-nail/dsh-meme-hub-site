...
- **启动方式**：本地 harness checkout（GitHub `dsh-v0.1.3-alpha.1`）`dsh web`；npm 发布线用 `npx -p @deepseek-ai/dsh@next dsh web`（lib 生产模式；勿 `install -g` 全局安装）
...
DSH 0.1.3-alpha.1 起，本插件可作为独立 bundle 一键安装到任意 profile（仓库位于 https://github.com/omdsh-dev，public）：
