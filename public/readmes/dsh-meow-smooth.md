...
支持 **dsh 0.1.5**（含最新的 `0.1.5-rc.1`），同时向下兼容旧版本——升级 dsh 不需要改本插件，也不需要改配置，手机端链接（tailscale serve 指向）照旧。

其中的手机访问加速代理会自动分辨宿主版本：老版 dsh 没有响应压缩，代理就压缩历史记录；新版 dsh 自己已经会压缩，代理自动切成纯透传，不再多绕一层。这个判断靠实测能力、不认版本号，所以夹在中间的版本也各有正确行为。
