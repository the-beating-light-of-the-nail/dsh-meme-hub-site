...
![Lujo-MCP Runtime Context Architecture](https://raw.githubusercontent.com/lujoai/Lujo-MCP/73d2dc4ab239e3ccb3f30f5d3e64c6310f6345e1/docs/public/images/lujo-runtime-context-architecture.svg)
...
**Lujo-MCP v0.7.0**（npm `@lujoai/lujo-mcp@0.7.0`，开箱即用）

> 版本统一：app / npm / README / CHANGELOG / MCP serverInfo / git tag 均为 `0.7.0`。
...
> **v0.7.0（2026-08-29）**：主题「稳定 + 可观测 + 债务清理」——**新增 KB 学习闭环可观测性**（Prometheus `kb_hits_total` / `kb_writeback_total` / `kb_experience_recall_total` 指标 + `GET /api/dashboard/kb-stats` + Dashboard「KB Learning Loop」面板，证明闭环在学）；Minor 大扫除两批 20 项（安全/健壮性 + 死代码/文档口径）；**integration 套件首次全绿**（113 tests / 0 failed）。unit **1409 tests / 0 failed**、SDK JS **47/47**、e2e 9 passed。**⚠️ 升级注意**：①npm 包要求 Node **>=18**（engines 收紧自 >=16）；②无 breaking change，配置全部向后兼容。详见 [RELEASE_NOTES](docs/public/RELEASE_NOTES.md)。
...
# → {"status":"ok","service":"Lujo-MCP","version":"0.7.0"}
```
...
| 测试基线    | 单元 `1403 passed / 6 skipped / 0 failed`（**1409 tests**，junit 权威计数）+ integration `96 passed / 17 skipped / 0 failed`（**113 tests，首次全绿**）+ Browser SDK JS `47 passed`；e2e `9 passed / 1 skipped`。历史演进：… → **1231**（v0.6.7）→ **1290**（P0+P1）→ **1298**（v0.6.8）→ **1386**（v0.6.9）→ **1409 tests**（v0.7.0） |
