...
### Referenced Files
Both global and workspace prompts can reference a **list of files**. Every assembly re-reads each referenced file with `fs.readFileSync` (UTF-8, live), sanitizes its content (spaces out `{` runs), and injects it after the prompt text. A read failure is **skipped** (that file's content is not injected) and reported in the UI with the reason. Supports plain text and markdown; no size cap. Per-file read status is projected to the UI via the `prompt-file-status` namespace (`ok`: N chars / `fail`: reason / pending).

### Session Auto-Resume
Sessions with the per-session switch on are resumed automatically after a GUI restart (`ctx.agents.resume`, carrying the default model from `agentDefaultModel`); switching a session on resumes it immediately (false→true edge). Filters: switch on, top-level only (no subagent origin, no `delegationDepth > 0`, no `parentSession`), non-blank (`seedLength !== 0`). Concurrency-bounded (`CONCURRENCY = 3`) with per-item failure isolation and an in-flight set that prevents duplicate resume.
...
The plugin targets **DeepSeek Harness `dsh-v0.1.2-alpha.1`**. The host- and client-side integration points it uses are present in that native version:

- **Framework**: `@deepseek-ai/cordis` 4.0.1 and `@deepseek-ai/schemastery` 3.18.1 (the harness vendors exactly these versions). The plugin loads through the cordis harness and registers as a bundle via `dsh.bundle.patch`.
- **Host services** (verified against the native source): `ctx.settings.register(ns, schema, { applies: 'live' })` → scope `{ get / watch(next, prev) / update / replace }` where `get()` returns a deep-frozen value; `ctx.systemPrompt.section({ name, order, text: (ctx) => ctx.agent?.session.header … })`; `ctx.agents.{ get, resume({ resumeSessionId, agentOptions }), roots, requireInitiator }`; `session.header` fields (`cwd`, `origin`, `delegationDepth`, `parentSession`, `seedLength`, `agentPreset`); `ctx.get('webServer').register({ kind: 'exact', path, handler })`; `@deepseek-ai/dsh-tools` `defineTool` + `tools.register()`; and `ctx.get('agentDefaultModel')`, `sessionPersistence`, `sessionTitle`, `workspaceRegistry`, `sessionLogDownload`, `timer`, `on`, `effect`.
...
The plugin's **host-side** `Config` validates the entire configuration tree — including everything under `config.client` — through schemastery as soon as the plugin loads. The client half reads that already-validated `config.client` and falls back to its own defaults per key. Because the native harness does **not** expose `@deepseek-ai/schemastery` to the client-side runtime module table (it is vendored inlined at build time with no shared identity), the client bundle does its own schemastery import inside a guarded fallback: if schemastery is unavailable, the client half degrades gracefully and does not export its own `Config` — configuration is still schema-validated on the host, so behavior and defaults are unaffected.
...
| `global-prompt` | `{ enabled: boolean, content: string, files: string[] }` | Injected into every conversation when enabled. `files` is the list of referenced files appended at assembly (live read, failed files skipped). |
...
The plugin exposes a single `Config` (schemastery schema) with per-feature keys. Defaults equal current behavior; override them via the plugin row's `config` in `cordis.yml` / `cordis.patch.yml` without touching code. The client half follows the same cordis mechanism (it receives `config.client`); if schemastery is unavailable inside the client bundle, the client half degrades to defaults and does not export its own `Config` — configuration is still validated on the host.
...
- **Client-side config schema** — because the native harness does not expose `@deepseek-ai/schemastery` to the client runtime module table, the client bundle validates its own config only when schemastery is available; otherwise validation happens on the host (see [Compatibility](#compatibility)).
