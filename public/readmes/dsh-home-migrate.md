...
![dsh-migrate 迁移流程总览](https://raw.githubusercontent.com/ygcdsj/dsh-home-migrate/4529c2a0d2f13d948ebe3db9f5b462c7d6407860/assets/migration-flow.svg)
...
![dsh-migrate 导入安全门禁](https://raw.githubusercontent.com/ygcdsj/dsh-home-migrate/4529c2a0d2f13d948ebe3db9f5b462c7d6407860/assets/security-gates.svg)
...
**为什么 `dsh web` 看不到迁移的插件？** `dsh web` 只是 `dsh --profile web` 的硬编码别名，启动的是原生的 `web` profile；迁移工具**不覆盖现有 profile**，成果都在新建的 `<name>-migrated` 里。请用 `dsh --profile <name>-migrated` 启动。确认迁移无误后，可停用旧 profile 并把 `<name>-migrated` 重命名为 `web`（`$DSH_HOME/profiles/` 下改目录名即可，DSH 无默认 profile 机制），此后 `dsh web` 即启动迁移后的环境。
