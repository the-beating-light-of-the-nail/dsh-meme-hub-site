...
<p align="center">
  <picture>
    ...
    <img src="https://raw.githubusercontent.com/HaoyueQin/dsh-better-reasoning-effort/9c8b3b4a1340b74b914049ae3556702199feb310/docs/banner.svg" alt="DSH Better Reasoning Effort" width="720">
  </picture>
</p>
...
<p align="center">
  <img src="https://raw.githubusercontent.com/HaoyueQin/dsh-better-reasoning-effort/9c8b3b4a1340b74b914049ae3556702199feb310/docs/demo.svg" alt="demo" width="640">
</p>

<p align="center">
  <img src="https://raw.githubusercontent.com/HaoyueQin/dsh-better-reasoning-effort/9c8b3b4a1340b74b914049ae3556702199feb310/assets/models-page-effort-editor.png" alt="The thinking-effort editor injected into a model row on the official Models page" width="720">
</p>
...
Requires DeepSeek Harness `0.1.1-rc.1` or newer, including the `0.1.2-alpha.1` through `0.1.2-alpha.5` pre-releases (`@deepseek-ai/dsh-api-remotes@^0.1.1-rc.1 || ^0.1.2-alpha.1 || ^0.1.2-alpha.2 || ^0.1.2-alpha.3 || ^0.1.2-alpha.4 || ^0.1.2-alpha.5`; the host half also peers on `@deepseek-ai/dsh-settings` with the same range and on `@deepseek-ai/schemastery@^3.18.0` — the explicit `||` branch is required because npm's prerelease exclusion rule makes a plain rc range reject `0.1.2-alpha.x`). The wire contract is verified against `0.1.1-rc.2` and statically re-checked against the `0.1.2-alpha.1` through `0.1.2-alpha.5` sources (settings Remote faces and the Models page DOM anchors; alpha.2 folded the `settingsNamespace` value export into a compile-time brand and re-coded the conflict error as `settings/conflict`; alpha.3's changes concentrate on the session/chat pipeline and leave the settings/Remote/DOM seats this plugin rides untouched; alpha.4's model discovery starts sending the profile's configured request headers — the probe route mirrors that composition — and its session-side breaking changes (indexed session reads, `SessionSeq`/`SessionLogOffset` brands) stay inside the session packages this plugin never imports; alpha.5 is a storage-layer release — session-projection-cache cross-version read compatibility plus the KV domain's `compatibleVersions`/`backup-and-skip` hardening — and touches none of the packages this plugin peers on); older release-candidate lines are not supported. The client bundle requests no official module at runtime, so it loads on the kernel lines unchanged.

**One DOM-bypass path for the per-model editor on ALL kernel lines (no version sniffing):** the injector keys off the official Capacity disclosure anchors (`Capacities`/`容量`), which are unchanged between rc.2, alpha.1, alpha.2, alpha.3, alpha.4 and alpha.5, so the editor mounts under every model row that expands — inside the *edit → custom settings* flow — including unsaved rows on a provider's create card (staged, flushed the moment the row is saved; alpha.1's create card works this way too, no "save first" dance needed). The only version-dependent seat is the slider toggle: on the alpha line (`0.1.2-alpha.1` through `0.1.2-alpha.5`) it rides the official `settings.models.footer` slot; on `0.1.1-rc.2` — whose Models section declares no extension slots — the same box is DOM-injected below the add actions. The switch itself keys off the settings wire seat the kernel exposes (`ctx.remote.settings` vs `connection.api`), not off a version string. The Models page's remaining sanctioned seat, the keyed `settings.models.provider-card` (per provider card, since alpha.1), is the migration path for card-level UI — but no slot reaches a single model row, which is why the per-model editor keeps the DOM bypass.
...
- **Maintained across kernels.** This is a reduced re-implementation over the harness wire contract (not a fork of the upstream bundle): it runs on `0.1.1-rc.2` and the `0.1.2-alpha.1` through `0.1.2-alpha.5` pre-releases without the upstream's `0.1.0-rc.6` pins, and the whole mount/unmount lifetime is managed by this plugin's DOM injector. If the upstream project resumes publishing, keep both in mind: running both plugins doubles up — the upstream shadows the official seat again, so the official trigger would disappear once more.
