...
| Surface | Status |
|---|---|
| Harness | DeepSeek Harness `0.1.2-rc.1` (peers `>=0.1.2-rc.1 <0.2.0`) (adapted 2026-09-02): the session envelope keeps its ignorable field for stored-log read compatibility only - Session.append still cannot stamp it, so audit-gate behavior is unchanged. |
| Node | `^22.19.0 \|\| >=24.0.0` |
