...
![dsh-read-url](https://raw.githubusercontent.com/2672243194/dsh-read-url/e5e01ee52cdff3c6a016107b1357857b0e49190e/docs/banner.svg)
...
| `maxPages` | number | 15 | Maximum attempted pages (2–50, including failures; fractional values round down) |
...
4. **Hidden-element and consent-banner stripping** — bounded attribute parsing recognizes `hidden`, `aria-hidden="true"`, `display:none`, `visibility:hidden` and `visibility:collapse`; balanced removal covers nested containers. Class names match word segments, preserving forum/download content and highlighted code; attributes accept quoted/unquoted values and whitespace around `=`;
...
11. **Compact fixed cost** — the four tool descriptions total 1,159 chars (audited by a budget assertion at 1,250 in tests, kept static for KV-cache); extended HTML-entity decoding (45 named entities) means leftovers like `&mdash;`/`&hellip;` never waste tokens or render as mojibake;
...
- **Content dispatch**: HTML, plain text/Markdown/CSV, JSON and vendor `+json` APIs, RSS 1.0/2.0 and Atom feeds. Feeds support namespace prefixes, relative links and `xml:base`; Atom prefers article links over self/enclosure links and preserves text constructs. JSON uses compact output with a 1500-character string cap. XML sitemaps return a clear unsupported-content error.
