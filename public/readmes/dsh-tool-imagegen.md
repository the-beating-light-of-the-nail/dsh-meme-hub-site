...
- **上传参考图（对话内上传通道）** — 对话框上传按钮（始终显示）：选择文件后**不立即提交**，先出现「已选图片」标记，等你输入消息后图文**一并发送**；模型收到信封里的图片路径，可基于它图生图。同一路径也服务多模态模型（走插件信封，而非平台原生 `image` 块）
...
（"已生成 1 张图片…"）            │
                                 ▼
                   <img src="https://raw.githubusercontent.com/Github-CJX/dsh-tool-imagegen/ac1a1bd582e6f5c0ad63d58632f40adccf938d89/api/dsh-tool-imagegen/attachment%3Fsession%3D%E2%80%A6%26id%3D%E2%80%A6">
...
| 上传按钮不见了 | 旧版用 `connection.api.sessions.models` 判断模型能力，该 RPC 在当前 DSH 上已不存在，判断永远抛错导致按钮被隐藏 | 升级到新版：上传按钮**始终显示**（不再做能力判断），纯文本与多模态模型走同一待发送通道 |
| 修改已生成图时气泡里出现 `【dsh-imagegen】…` 长段说明文字 | 模型侧信封文本出现在用户气泡 | 新版对用户气泡渲染做了形状兜底，确保过滤生效：用户只看到图片与自己的话；信封仅模型可见 |
| 生成图收进可折叠的通用工具行（要点开才见） | 工具视图对调用块形状判断失败而崩溃，keyed 视图被剔除回退到通用卡片 | 升级到新版：工具视图兼容 `content` 与嵌套 `result.content` 两种块形态，任何形状都不会崩溃回退 |
...
| 生成图时对话里多出一张毫不相干的图片 | c7 的 markdown 渲染器开始渲染 `![...](https://raw.githubusercontent.com/Github-CJX/dsh-tool-imagegen/ac1a1bd582e6f5c0ad63d58632f40adccf938d89/http(s)://...)`；模型从信封得知「图片已生成」后自己写 markdown 图片引用（URL 为模型幻觉） | 升级插件到最新版（信封与系统提示已明确告知模型「图片已直接显示，请勿在回复中引用图片」） |
