...
- **Plan approval** — Under read-only access Claude works in plan mode, and the plan it hands back arrives in the approval surface as the prose it was written as rather than as tool arguments, so the decision is made against something readable. The turn's tool card carries the same plan.
