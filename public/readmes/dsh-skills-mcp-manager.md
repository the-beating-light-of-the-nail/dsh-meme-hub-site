...
前置条件：Node.js >= 22.19，并先装好 dsh 命令行。
    ...
    dsh plugin --profile web add @zebbkira/dsh-skills-mcp-manager@0.2.0

    # 3. 重启 dsh web
    dsh web

dsh plugin 会把包装进 profile 并自动把它加入插件层（本包声明了 dsh.bundle.patch），无需手动改任何配置。版本号 @0.2.0 可换成 npm 上的最新版。
