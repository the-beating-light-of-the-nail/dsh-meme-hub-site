...
### 2026-09-10 起 flash 系列降价（v1.4.1）

**北京时间 2026-09-10 12:00 起**，flash 系列（含 `deepseek-v4-flash-vision-exp`）
按新价计费（高峰 = 空闲 ×2）；pro 系列本次未调整，仍按上表价格：

| flash 时段 | 缓存命中输入 | 缓存未命中输入 | 输出 |
|---|---|---|---|
| 高峰（周一至五 9–12 / 14–18） | ¥0.04 | ¥2.0 | ¥8.0 |
| 空闲（其余时间，含周末全天） | ¥0.02 | ¥1.0 | ¥4.0 |

2026-09-10 12:00 之前的 flash 事件仍按上一时段费率（¥0.10/¥0.05 命中、
¥3.0/¥1.5 未命中、¥9.0/¥4.5 输出）；**当天中午 12:00 前后分界**，上午旧价、
下午新价各自独立记账，不会混算。周末空闲规则不变（高峰窗口仍仅周一至周五）。
