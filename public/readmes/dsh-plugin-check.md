...
注册 `plugin_check` 工具（`@omdsh-dev/dsh-plugin-check`，row id `tool-plugin-check`），统一输出 JSON 文本。
...
仓库位于 [omdsh-dev/dsh-plugin-check](https://github.com/omdsh-dev/dsh-plugin-check)（public）。将本插件作为独立 bundle 安装到 profile（DSH 0.1.3-alpha.1（npm））：
