...
## DSH 0.1.3-alpha.1 兼容（已验证）

本插件已迁移到 DSH 0.1.3-alpha.1 依赖线，并在 `local harness 0.1.3-alpha.1` 的隔离 consumer 中完成全链路验证：
...
- **消费验证**：tarball 装入 0.1.3-alpha.1 consumer → `dsh --profile compat --dump-config` 出现本插件 row → 工具真实注册与执行通过
- **启动方式**：`npx -p @deepseek-ai/dsh@next dsh web`（lib 生产模式；勿 `install -g` 全局安装）


## 版本适配

- **适配 DSH**: DSH 0.1.3-alpha.1（npm）（迁移：profile/bundle 插件系统）
- **bundle 声明**: `package.json` 的 `dsh.bundle`（patch 指向 `cordis.patch.yml`）+ `exports` 导出
- **patch 格式**: `cordis.patch.yml` 使用 `- insert:` 列表（DSH 0.1.3-alpha.1（npm）的 patch 是 id-targeted 语义，裸 `- id:` 条目会报 `entry not found`）
...
DSH 0.1.3-alpha.1（npm）起，本插件可作为独立 bundle 一键安装到任意 profile（仓库位于 https://github.com/omdsh-dev，public）：
...
> DSH 0.1.3-alpha.1（npm）注意：patch 是 id-targeted 语义——裸 `- id:` 条目会报 `entry "xxx" not found`，必须用 `- insert:` 列表包裹。
## 用法
...
1. **分发链路**：`@deepseek-ai/dsh-tools` 已随 DSH 0.1.3-alpha.1（npm）发布为私有 npm 包；插件经 `dsh plugin add github:omdsh-dev/...` 或 tarball 直接安装，无需放入 monorepo 走 workspace 解析
