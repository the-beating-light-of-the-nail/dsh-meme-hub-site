...
- **Fault isolation (the shell)**: the DSH loader mounts all patch rows as one transactional group — a single plugin that fails to import or start would roll back the whole group and abort `dsh web`. The aggregate therefore mounts every family plugin behind a never-failing shell module (this package's main entry): the row `name` points at `@linxin666/dsh-web-all` and the row `config` names the real plugin package. A broken plugin now degrades alone (logged, and listed by the loopback-only health route `GET /api/dsh-web-all/degraded`) while every other plugin mounts normally. External rows (npm packages outside the family) keep mounting directly; `dsh-i18n` stays direct (empty host half).
