...
### 1. 🌐 100% Покрытие ядра DSH (30+ Пространств имён, 720+ ключей)
...
dsh-russian-lang:
  typography:
    enabled: true
    quotes: true         # Преобразование кавычек в «ёлочки»
    dashes: true         # Длинные тире
    nbsp: true           # Неразрывные пробелы
  layoutFix:
    enabled: true
    hotkey: "Alt+KeyL"   # Горячая клавиша исправления раскладки
  spellcheck: true       # Браузерная проверка правописания
  overrides: {}          # Пользовательские переопределения ключей
```
