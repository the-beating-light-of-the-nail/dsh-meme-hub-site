...
dsh plugin --profile web add '@dsh-external/dsh-minigames@github:lhh010/dsh-minigames#v0.3.18'
...
> 1. 执行 `dsh plugin --profile web add '@dsh-external/dsh-minigames@github:lhh010/dsh-minigames#v0.3.18'`（首次可能被 pnpm 11 拦截 node-pty 构建脚本而失败）
...
### 2026-09-09 · v0.3.18 — 声明支持 dsh-v0.1.5-alpha.2
- **验证**：alpha.2 改动为 Sidebar 文档预览、模型文件交付、minimal 默认工具调整与 `fs-ext` 安装修复，client 插件面零代码差异；npm 已发布，钉版本实机验证；typecheck/build/203 单测全绿，启动清单确认加载
### 2026-09-08 · v0.3.17 — 声明支持 dsh-v0.1.5-alpha.1
...
### 2026-09-05 · v0.3.16 — 声明支持 dsh-v0.1.3-alpha.2

- **验证**：alpha.2 改动全在 pi-ai / Web 顶栏 / 子代理消息 / host 面，client 插件面零代码差异；npm 已发布，钉版本实机验证；typecheck/build/单测全绿

### 2026-09-04 · v0.3.15 — 声明支持 dsh-v0.1.3-alpha.1

- **验证**：0.1.3 破坏性变更集中在 host/session 侧（SessionHandle / session format v2），client 插件面零代码差异；npm 未发布，源码宿主实机验证；typecheck/build/203 单测全绿

### 2026-09-03 · v0.3.14 — 声明支持 dsh-v0.1.2-rc.1

- **验证**：alpha.5→rc.1 为纯版本号提交（252 文件零代码差异）；实机 rc.1 验证通过，无需代码改动
...
- **根因**：方块落到最终位置后并不立即合并，要等下一次重力 tick 失败才锁定+消除——1 级时最长悬停 **800ms**，期间拼满的行看起来「该消除却没消除」；而高级别（120ms）又快到无法把方块滑进最左列，留下永远差一格的死列
- **修复**：引入标准**锁定延迟（lock delay）**——落地后保留 400ms 可控窗口（可滑动/旋转微调，滑动与旋转会刷新窗口，每块最多刷新 15 次防无限拖延），到期自动锁定并立即执行消除；空格硬降/↓ 软降到底仍是即时锁定。同时在 `lock()` 内加入自检：满行若在消除后仍残留会输出错误日志（正常情况下不可能发生）
- **验证**：新增 `isLanded`/`lock` 单测 2 例（含消除后守恒断言），203 例全部通过；typecheck、build 通过

### 2026-08-20 · v0.3.5 — 声明 DSH 0.1.1-rc.1 兼容性（实机 boot 验证）

- **验证**：DSH npm `0.1.1-rc.1` 实机 boot 验证通过——boot 清单包含 `@dsh-external/dsh-minigames`、client.js 返回 200；本插件为纯浏览器端 bundle（`document.body` portal），不依赖任何宿主服务或布局槽位，0.1.1-rc.1 上零改动，全部 18 款游戏行为不变

### 2026-08-20 · v0.3.4 — 修复黑白棋 HUD 棋子符号反向

- **修复**：黑白棋 HUD 与终局比分的棋子符号与直觉相反——此前实心 `●` 配的是黑棋（玩家）计数、空心 `○` 配的是白棋（AI）计数，读起来像比分/胜负算错了。现改为 `○`（空心）= 黑棋（玩家）、`●`（实心）= 白棋（AI），与棋盘实际棋子一致；落子/翻转/终局判定逻辑不变（已用 300 局随机模拟验证）。
...
- 恐龙雨天/昼夜为纯视觉氛围，不影响判定。
`v0.3.19`（默认） | `dsh-v0.1.2-alpha.1`~`alpha.5`、`rc.1`、`0.1.3-alpha.1`~`0.1.5-rc.1` | 声明支持 0.1.5-rc.1（npm 已发布，钉版本实机验证；rc.1 为 0.1.5 系列首个候选版本，client 插件面零代码差异；typecheck/build/203 单测全绿，启动清单确认加载） |
